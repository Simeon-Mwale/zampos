...
# Cache TTL: refresh every 30-60 seconds
RATE_CACHE_TTL = int(os.getenv("RATE_CACHE_TTL_SECONDS", "45"))

# In-memory cache (private to this module)
_rate_cache: Dict = {
    "zmw_per_btc": None,
    "sats_per_zmw": None,
    "fetched_at": None,
...
def get_cache_metadata() -> Dict:
    """
    Public helper to expose cache metadata safely.
    Use this from router.py instead of accessing _rate_cache directly.
    """
    return {
...
async def _fetch_usd_zmw_rate() -> Optional[Decimal]:
    """Fetch USD/ZMW FX rate from configured API or fallback."""
    # Try configured FX API first
    ...
    # Fallback: Use CoinGecko's direct ZMW support
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            resp = await client.get(
                f"{COINGECKO_URL}/simple/price",
            ...
            if btc_usd > 0 and btc_zmw > 0:
                ...
                logger.info(f"💱 Fallback (CoinGecko): 1 USD = {usd_zmw:.4f} ZMW")
                return usd_zmw
    except Exception as e:
...
async def fetch_live_rates(force_refresh: bool = False) -> Tuple[Decimal, Decimal]:
    """
    Fetch live ZMW/BTC and sats/ZMW rates.
    
    Returns:
        tuple: (zmw_per_btc, sats_per_zmw) as Decimals
    """
    ...
    logger.debug("🔄 Cache MISS — fetching fresh rates")
    ...
    if not btc_usd or not usd_zmw:
        # Return last known good rates or safe defaults
        if _rate_cache["zmw_per_btc"] and _rate_cache["sats_per_zmw"]:
            logger.warning("⚠️ Using cached rates due to API failure")
            return _rate_cache["zmw_per_btc"], _rate_cache["sats_per_zmw"]
        # Hardcoded safe fallback (update periodically!)
        logger.error("❌ All rate sources failed - using emergency fallback")
        emergency_zmw_per_btc = Decimal("1500000")
        emergency_sats_per_zmw = (SATOSHI_PER_BTC / emergency_zmw_per_btc).quantize(Decimal("0.00000001"), rounding=ROUND_DOWN)
        return emergency_zmw_per_btc, emergency_sats_per_zmw
    
    # Calculate: ZMW per BTC = (USD per BTC) * (ZMW per USD)
    zmw_per_btc = (btc_usd * usd_zmw).quantize(Decimal("0.01"), rounding=ROUND_DOWN)
    
    # Calculate: sats per ZMW = 100,000,000 / (ZMW per BTC)
    sats_per_zmw = (SATOSHI_PER_BTC / zmw_per_btc).quantize(Decimal("0.00000001"), rounding=ROUND_DOWN)
    ...
    logger.info(f"✅ Rates updated: 1 BTC = {zmw_per_btc:,.2f} ZMW | 1 ZMW = {sats_per_zmw:.8f} sats")
    return zmw_per_btc, sats_per_zmw


def zmw_to_sats(zmw_amount: float, zmw_per_btc: Optional[Decimal] = None) -> int:
    """
    Convert ZMW amount to satoshis with precision-safe math.
    
    Args:
        zmw_amount: Amount in Zambian Kwacha
        zmw_per_btc: Optional pre-fetched rate (uses live fetch if None)
    
    Returns:
        int: Amount in satoshis (minimum 1)
    """
    try:
        ...
        # Use provided rate or fetch live
        rate = zmw_per_btc
        if rate is None:
            # For sync contexts, use cached value only
            if _is_cache_valid():
                rate = _rate_cache["zmw_per_btc"]
            else:
                return 0
        
        # Calculate: sats = (ZMW / (ZMW/BTC)) * 100,000,000
