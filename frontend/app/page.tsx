// app/page.tsx — ZamPOS v2.1: Direct + Custodial payout modes
'use client'
import { useState, useEffect, useCallback, useRef } from 'react'
import { Zap, RefreshCw, ChevronRight, X, CheckCircle, Clock, LayoutDashboard,
...
export default function POSPage() {
  ...
  const _onPaymentSuccess = () => {
    // Update custodial balance locally if custodial mode
  ...
  const handleCharge = async () => {
    ...
    try {
      const inv = await createInvoice(zmwAmount, memo || 'ZamPOS Payment', getMerchantId())
      setInvoice(inv); setScreen('invoice')
    } catch (err: any) {
  ...
  const handleNewSale = () => {
    setZmwInput(''); setMemo(''); setInvoice(null); setError(''); setRateWarning(null)
    setScreen('pos')
  }
                  ...
                  <button key={mode} onClick={() => setPayoutMode(mode)}
                    ...
                    <p className="text-muted text-xs mt-1 font-body">
                      {mode === 'direct'
                        ...
                        : 'Accumulate & withdraw when ready'}
                    </p>
                  </button>
                ))}
              </div>
